#include "Binary_tree_13.h"

Graph_lib::Binary_tree_13::Binary_tree_13(Point xy, int levels, Connector_type con)
	: lvl(levels)
{
	// Determine nodes
...
void Graph_lib::Binary_tree_13::draw_lines() const
{
	if (color().visibility()) {
...
void Graph_lib::Binary_tree_13::move(int dx, int dy)
{
	Shape::move(dx, dy);
	for (int i = 0; i < edges.size(); ++i)
	{
